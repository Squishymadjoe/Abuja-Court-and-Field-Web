...
const Home: React.FC<HomeProps> = ({ setPage, onPlay }) => {
  return (
    <div className="relative flex w-full flex-col">
      ...
      {/*
        BOLT ⚡: Performance Optimization
        - WHAT: Replaced CSS background-images with `<img>` tags for all below-the-fold images.
        - WHY: Unlike background images, `<img>` tags support the `loading="lazy"` attribute. This instructs the browser to defer loading these images until they are about to enter the viewport, instead of loading them all during the initial page load.
        - IMPACT: This reduces the initial network payload, decreases contention for bandwidth with more critical resources, and speeds up the initial `load` and `DOMContentLoaded` events. It directly improves metrics like First Contentful Paint (FCP) and Time to Interactive (TTI).
        - MEASUREMENT: The improvement can be verified by observing the network waterfall in browser developer tools. Fewer image requests will be made on initial load, and new requests will appear as the user scrolls down.
      */}
             ...
             {EPISODES.slice(0, 4).map((episode) => (
               <div key={episode.id} className="group relative min-w-[300px] flex-1 max-w-[400px] snap-center rounded-2xl bg-surface-dark p-4 transition-transform hover:-translate-y-1">
                 <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-black">
                   <img src={episode.image} alt={episode.title} loading="lazy" className="h-full w-full object-cover object-center transition-transform duration-500 group-hover:scale-110" />
