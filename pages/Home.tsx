    ...
    <div className="relative flex w-full flex-col">
      ...
      <section className="relative flex min-h-[90vh] w-full flex-col items-center justify-center overflow-hidden px-4 pt-20 pb-12">
        <div className="absolute inset-0 z-0">
          {/* OPTIMIZATION: Replaced CSS background image with `<img>` tag for better LCP.
              This is an above-the-fold hero image, so we omit `loading="lazy"` to ensure
              it's prioritized and loaded as quickly as possible. */}
          <img
            ...
            alt="Abuja basketball and flag football leagues collage"
             ...
             {EPISODES.slice(0, 4).map((episode) => (
               <div key={episode.id} className="group relative min-w-[300px] flex-1 max-w-[400px] snap-center rounded-2xl bg-surface-dark p-4 transition-transform hover:-translate-y-1">
                 <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-black">
                   {/* OPTIMIZATION: Replaced CSS background image with `<img>` and added `loading="lazy"`
                      because these images are below the fold. This defers loading of off-screen
                      images, improving initial page load performance. */}
                   <img
                    src={episode.image}
                    alt={episode.title}
                    loading="lazy"
