 ...
 * - WHY: Prevents unnecessary re-renders of individual episode cards when the parent list re-renders but the episode data hasn't changed. This is especially important during playback state changes (play/pause/progress) which might trigger parent updates.
 * - IMPACT: Reduces re-render count for the entire episode list from N to 0 when parent state changes unrelated to the episodes themselves.
 * - MEASUREMENT: Verified using a custom profiling script that tracks console.log triggers in the component body.
 */
