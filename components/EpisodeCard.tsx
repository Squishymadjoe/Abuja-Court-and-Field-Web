    ...
    <article className="group relative flex flex-col overflow-hidden rounded-2xl bg-surface-dark border border-white/5 transition-all hover:-translate-y-1 hover:border-primary/50 hover:shadow-[0_0_30px_-10px_rgba(0,255,0,0.2)]">
      <div className="aspect-video w-full overflow-hidden bg-[#204b20] relative">
        {/*
          ...
          - WHAT: Replaced CSS background-image with an <img> tag.
          - WHY: Enables the browser's preload scanner to discover images early, improving Largest Contentful Paint (LCP).
          - IMPACT: Faster visual loading of episode cards.
        */}
        <img
          src={episode.image}
          alt={episode.title}
 ...
 * - WHY: Prevents re-rendering the card when its props (episode, onPlay) haven't changed.
 * - IMPACT: When toggling playback in the parent, all 6 cards skip re-rendering.
 */
