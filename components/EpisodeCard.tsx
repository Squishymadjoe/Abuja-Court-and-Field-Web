    ...
    <article className="group relative flex flex-col overflow-hidden rounded-2xl bg-surface-dark border border-white/5 transition-all hover:-translate-y-1 hover:border-primary/50 hover:shadow-[0_0_30px_-10px_rgba(0,255,0,0.2)]">
      <div className="aspect-video w-full overflow-hidden bg-[#204b20] relative">
        {/*
          ...
          - WHAT: Replaced CSS background-image with an <img> tag and added loading="lazy".
          - WHY: <img> tags with loading="lazy" allow the browser to defer loading of images that are not yet in the viewport, saving bandwidth and improving initial page load time. object-fit: cover maintains the visual style.
          - IMPACT: Reduces initial network payload and speeds up the first paint for pages with many episodes.
        */}
        <img
          src={episode.image}
          alt={episode.title}
