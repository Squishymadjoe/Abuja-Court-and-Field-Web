...
/*
  ...
  - WHAT: Wrapped EpisodeCard in React.memo and replaced background-image with an <img> tag using loading="lazy".
  - WHY:
    1. React.memo prevents unnecessary re-renders when the parent component updates but the episode props remain the same.
    2. Replacing CSS background-image with a native <img> tag allows for browser-level optimizations like lazy loading (loading="lazy"), which reduces initial bandwidth and improves LCP.
  - IMPACT: Reduces re-renders to zero when playing/pausing episodes and improves page load speed by deferring off-screen images.
  - MEASUREMENT: Re-renders verified with console instrumentation. Image lazy loading can be verified in the Network tab.
*/
