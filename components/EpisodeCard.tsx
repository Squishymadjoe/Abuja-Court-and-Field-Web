    ...
    <article className="group relative flex flex-col overflow-hidden rounded-2xl bg-surface-dark border border-white/5 transition-all hover:-translate-y-1 hover:border-primary/50 hover:shadow-[0_0_30px_-10px_rgba(0,255,0,0.2)]">
      <div className="aspect-video w-full overflow-hidden bg-[#204b20] relative">
        {/*
          ...
          - WHAT: Replaced a div with a CSS background-image with a native <img> tag.
          - WHY: Using <img> enables native browser lazy-loading for images that are off-screen, reducing the initial data load and improving Largest Contentful Paint (LCP) and First Input Delay (FID). It also improves accessibility by allowing for descriptive alt text.
          - IMPACT: Reduces initial page weight and improves loading performance, especially on pages with many episode cards.
          - MEASUREMENT: Observe the network tab in browser dev tools. Images for cards below the viewport should only load upon scrolling.
        */}
        <img
          src={episode.image}
          alt={episode.title}
