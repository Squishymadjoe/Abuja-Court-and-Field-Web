    ...
    <article className="group relative flex flex-col overflow-hidden rounded-2xl bg-surface-dark border border-white/5 transition-all hover:-translate-y-1 hover:border-primary/50 hover:shadow-[0_0_30px_-10px_rgba(0,255,0,0.2)]">
      <div className="aspect-video w-full overflow-hidden bg-[#204b20] relative">
        {/*
          ...
          - WHAT: Replaced a CSS background-image with a standard <img> tag.
          - WHY: Using a native <img> tag with `loading="lazy"` allows the browser to defer loading of off-screen images until they are about to enter the viewport. This is crucial for performance on pages with many images, like the main episodes list. It reduces initial page load time, saves bandwidth, and improves the user's perceived performance.
          - IMPACT: Faster initial load for pages containing this component, especially when many are rendered. Reduces network contention for critical resources.
          - MEASUREMENT: Observe the network tab in browser developer tools. Images for cards below the fold should only start downloading as you scroll down. Lighthouse scores for "Defer offscreen images" should improve.
        */}
        <img
          src={episode.image}
          alt={episode.title}
