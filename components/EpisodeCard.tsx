    ...
    <article className="group relative flex flex-col overflow-hidden rounded-2xl bg-surface-dark border border-white/5 transition-all hover:-translate-y-1 hover:border-primary/50 hover:shadow-[0_0_30px_-10px_rgba(0,255,0,0.2)]">
      <div className="aspect-video w-full overflow-hidden bg-[#204b20] relative">
        {/*
          ...
          - WHY: Using a standard `<img>` tag allows for native browser lazy loading (`loading="lazy"`), which defers loading the image until it is close to the viewport. This is critical for performance on pages that list many episodes, as it prevents loading all images at once.
          - IMPACT: Significantly improves initial page load speed and reduces bandwidth usage on the episodes page.
        */}
        <img
          src={episode.image}
          alt={episode.title}
