...
const EpisodeCard: React.FC<EpisodeCardProps> = React.memo(({ episode, onPlay }) => {
  /*
    ...
    - WHY: Prevents unnecessary re-renders of the episode card when the parent list re-renders
           but the episode data and play callback (stabilized via useCallback) remain the same.
    - IMPACT: Reduces the total number of re-renders during playback state changes.
  */
  return (
